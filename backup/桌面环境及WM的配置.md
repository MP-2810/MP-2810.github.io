...
如果是旧版Hyprland,则删除`hyprland.conf`中的`autogenerated = 1`一行
...
- 如果有多个显示器，由于将主显示器的坐标位置设置成了（0,0），按照主显示器`横向（竖向规模）/缩放倍率`的数值，将位置信息按坐标填写。如我的笔记本屏幕要放在外接显示器左边，我的主显示器横向规模是2560,缩放倍率是1.00，则填写`-2560x0`